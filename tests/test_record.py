...
class TestBuildRecord(unittest.TestCase):
    def test_schema_version_is_current(self):
        self.assertEqual(_record()["schema_version"], "0.6")
        self.assertEqual(_record()["schema_version"], record.SCHEMA_VERSION)
...
class TestParseRecord(unittest.TestCase):
    ...
    def test_unknown_schema_version_raises(self):
        for version in ("0.3", "0.4", "0.7", None):
            rec = _record()
            rec["schema_version"] = version
            with self.assertRaises(record.RecordParseError):
                record.parse_record(json.dumps(rec))

    def test_05_record_is_upgraded_on_read(self):
        rec = _record(source_type="derived")
        rec["schema_version"] = "0.5"
        rec["derived_from"] = "rs2/csac/amber/1_interim/csac-clean"
        upgraded, flag = record.parse_record_with_status(json.dumps(rec))
        self.assertTrue(flag)
        self.assertEqual(upgraded["schema_version"], "0.6")
        self.assertEqual(upgraded["derived_from"], [
            {"kind": "dataset", "identifier": "rs2/csac/amber/1_interim/csac-clean"}])
        # Field order survives: derived_from stays where 0.5 put it.
        self.assertEqual(list(upgraded).index("derived_from"),
                         list(rec).index("derived_from"))
        errors, _ = record.validate_record(upgraded, {"armed-conflict", "forced-labour"})
        self.assertEqual(errors, [])

    def test_current_record_is_not_flagged_as_upgraded(self):
        rec = _record()
        same, flag = record.parse_record_with_status(record.record_json(rec))
        self.assertFalse(flag)
        self.assertEqual(same, rec)

    def test_05_empty_derived_from_is_dropped(self):
        rec = _record()
        rec["schema_version"] = "0.5"
        rec["derived_from"] = "  "
        upgraded, _ = record.parse_record_with_status(json.dumps(rec))
        self.assertNotIn("derived_from", upgraded)


REAL_RECORDS = Path(__file__).resolve().parent / "data" / "records"


class TestRealCorpusUpgrade(unittest.TestCase):
    """Three 0.5 records generated by cdisaw-parquet: a plain URL, a URL
    with a query string, a URL with a fragment. All must upgrade to one
    external reference and validate."""

    def test_each_fixture_upgrades_and_validates(self):
        fixtures = sorted(REAL_RECORDS.glob("dataset.*.json"))
        self.assertEqual(len(fixtures), 3)
        from crsw_deposit import vocab
        vocab_dict = json.loads(vocab.bundled_vocab_path().read_text(encoding="utf-8"))
        for path in fixtures:
            text = path.read_text(encoding="utf-8")
            original = json.loads(text)
            self.assertEqual(original["schema_version"], "0.5")
            rec, upgraded = record.parse_record_with_status(text)
            self.assertTrue(upgraded, path.name)
            self.assertEqual(rec["derived_from"],
                             [{"kind": "external", "url": original["derived_from"]}],
                             path.name)
            errors, _ = record.validate_record(
                rec, vocab.all_terms(vocab_dict), vocab.domain_codes(vocab_dict))
            self.assertEqual(errors, [], path.name)
            # Written back, it is a 0.6 record that re-reads as itself.
            again, flag = record.parse_record_with_status(record.record_json(rec))
            self.assertFalse(flag)
            self.assertEqual(again, rec)


class TestReferences(unittest.TestCase):
    def test_identifier_becomes_dataset_reference(self):
        self.assertEqual(
            record.reference_from_text(" rs2/csac/amber/1_interim/csac-clean "),
            {"kind": "dataset", "identifier": "rs2/csac/amber/1_interim/csac-clean"})

    def test_urls_become_external_url(self):
        for url in ("https://github.com/jrnold/CDB90",
            ...
            self.assertEqual(record.reference_from_text(url),
                             {"kind": "external", "url": url}, url)

    def test_anything_else_is_a_citation(self):
        text = "Arnold, J. (2014). CDB90. GitHub."
        self.assertEqual(record.reference_from_text(text),
                         {"kind": "external", "citation": text})
        self.assertEqual(record.reference_from_text("rs2/csac/amber"),
                         {"kind": "external", "citation": "rs2/csac/amber"})

    def test_validate_reference(self):
        own = "rs2/csac/green/2_final/x"
        ok = {"kind": "dataset", "identifier": "rs2/csac/amber/1_interim/y",
              "dataset_uuid": "8f14e45f-ceea-467f-a34e-9db1c153f0a1", "version": "1-0"}
        self.assertEqual(record.validate_reference(ok, "r", own), [])
        self.assertEqual(record.validate_reference(
            {"kind": "external", "url": "https://x.org"}, "r", own), [])
        self.assertEqual(record.validate_reference(
            {"kind": "external", "citation": "Someone 2001"}, "r", own), [])
        self.assertTrue(record.validate_reference("text", "r", own))
        self.assertTrue(record.validate_reference({"kind": "book"}, "r", own))
        self.assertTrue(record.validate_reference(
            {"kind": "dataset", "identifier": "not/five/parts"}, "r", own))
        self.assertIn("derived from itself", record.validate_reference(
            {"kind": "dataset", "identifier": own}, "r", own)[0])
        self.assertTrue(record.validate_reference(
            {"kind": "dataset", "identifier": "rs2/csac/amber/1_interim/y",
             "dataset_uuid": "nope"}, "r", own))
        self.assertTrue(record.validate_reference(
            {"kind": "dataset", "identifier": "rs2/csac/amber/1_interim/y",
             "version": "v1"}, "r", own))
        self.assertTrue(record.validate_reference({"kind": "external"}, "r", own))
        self.assertTrue(record.validate_reference(
            {"kind": "external", "url": "u", "retrieved": "2026"}, "r", own))
        self.assertEqual(record.validate_reference(
            {"kind": "external", "url": "u", "retrieved": "2026-09-17"}, "r", own), [])

    def test_record_level_checks(self):
        terms = {"armed-conflict", "forced-labour"}
        rec = _record(derived_from="a string")
        errors, _ = record.validate_record(rec, terms)
        self.assertTrue(any("0.5 form" in e for e in errors))
        rec = _record(derived_from=[{"kind": "dataset", "identifier": RECORD_FIELDS["identifier"]}])
        errors, _ = record.validate_record(rec, terms)
        self.assertTrue(any("derived from itself" in e for e in errors))
        rec = _record(source_type="derived")
        errors, warnings = record.validate_record(rec, terms)
        self.assertEqual(errors, [])
        self.assertTrue(any("derived from what" in w for w in warnings))
        rec = _record(derived_from=[{"kind": "external", "citation": "x"}])
        errors, warnings = record.validate_record(rec, terms)
        self.assertEqual(errors, [])
...
class TestActivities(unittest.TestCase):
    ...
    def good(self, **overrides):
        act = {"activity": "harmonise",
               "description": "ingest script run against the Parquet shim",
               ...
               "agent": "k1078591",
               "started": "2026-09-17T08:40:00Z", "ended": "2026-09-17T08:59:10Z"}
        act.update(overrides)
        return act

    def test_good_activity(self):
        self.assertEqual(record.validate_activity(self.good(), "p", self.PATHS),
                         ([], []))

    def test_unknown_activity_kind(self):
        errors, _ = record.validate_activity(self.good(activity="teleport"), "p", self.PATHS)
        self.assertTrue(any("teleport" in e for e in errors))
        errors, _ = record.validate_activity(
            self.good(activity="reproject"), "p", self.PATHS,
            activity_codes=["reproject", "harmonise"])
        self.assertEqual(errors, [])

    def test_outputs_must_be_members(self):
        errors, _ = record.validate_activity(
            self.good(outputs=["missing.parquet"]), "p", self.PATHS)
        self.assertTrue(any("missing.parquet" in e for e in errors))

    def test_bare_path_inputs_must_be_members(self):
        errors, _ = record.validate_activity(
            self.good(inputs=["wide/events/cdb90.parquet"]), "p", self.PATHS)
        self.assertEqual(errors, [])
        errors, _ = record.validate_activity(self.good(inputs=["nope"]), "p", self.PATHS)
        self.assertTrue(errors)

    def test_commit_shape_is_a_warning_only(self):
        errors, warnings = record.validate_activity(
            self.good(tool={"name": "t", "commit": "main"}), "p", self.PATHS)
        self.assertEqual(errors, [])
        self.assertTrue(any("commit" in w for w in warnings))

    def test_tool_needs_a_name_and_timestamps_are_utc(self):
        errors, _ = record.validate_activity(self.good(tool={"repo": "r"}), "p", self.PATHS)
        self.assertTrue(errors)
        errors, _ = record.validate_activity(self.good(started="yesterday"), "p", self.PATHS)
        self.assertTrue(errors)

    def test_manual_activity_with_nothing_else(self):
        self.assertEqual(record.validate_activity({"activity": "manual"}, "p", set()),
                         ([], []))

    def test_record_level(self):
        terms = {"armed-conflict", "forced-labour"}
        rec = _record(provenance=[{"activity": "clean", "outputs": ["csac-clean-2025.csv"]}])
        self.assertEqual(record.validate_record(rec, terms)[0], [])
        rec = _record(provenance=[{"activity": "clean", "outputs": ["other.csv"]}])
        self.assertTrue(record.validate_record(rec, terms)[0])
        rec = _record(provenance="not a list")
        self.assertTrue(record.validate_record(rec, terms)[0])
        self.assertIn("provenance", rec)
        self.assertNotIn("provenance", _record(provenance=[]))
