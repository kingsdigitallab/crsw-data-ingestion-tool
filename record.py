...
# --- legacy v0.3 per-file sidecar API ----------------------------------
# Retired by r5 (§1 Q2). Kept only so the module rename lands green;
# deleted when deposit.py moves to the dataset record.

SIDECAR_SCHEMA_VERSION = "0.3"
...
def build_sidecar(object_key, strand, domain, project, state, sensitivity,
    ...
    """Assemble the v0.3 sidecar dict in old spec §5 field order.
    Recommended/optional fields that are None or empty are omitted."""
    sc = {
...
def validate_sidecar(sc: dict, vocab_terms: Set[str],
    ...
    """Return (errors, warnings) for a v0.3 sidecar."""
    errors = []
    warnings = []
    for field in _SIDECAR_REQUIRED:
