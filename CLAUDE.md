...
- `crsw_web/` read role (off unless `CRSW_READ_S3_*` are set): `catalogue.py` reads the promoter's index through `s3.ReadOnly` (get/head/list only, never widen it), `access.py` is the one amber rule, routes `/datasets`, `/datasets.json`, `/datasets/{identifier}`, `/datasets/{identifier}/record`. The upload handlers never touch the read client.
...
- **Separate promoter** inside the KCL network with the real key. Automated: every complete deposit whose checks all pass is promoted; anything amiss is reported and left in staging (decision of 18 Sept 2026, replacing the plan's "human approval by default"). `--dry-run` reviews without moving. Every action is a JSON line in the log, and each run's lines are also written as one object under `PROMOTER_AUDIT_PREFIX` (`audit/promoter/YYYY/MM/DD/<run-id>.jsonl`) before exit, the durable audit trail; `promoter audit` and `promoter datasets` read the bucket back. After a real run that changed a record it also rewrites the index under `PROMOTER_INDEX_PREFIX` (`index/datasets.jsonl` + `.parquet`, one row per record in place; `promoter/index.py`; `promoter index` rebuilds it), which the service's Find data page, the form's picker and cdisaw-parquet read. A deposit's `derived_from` references to other datasets get the parent's uuid and version filled in before promotion (`promoter/resolve.py`, r8 §4). Checks: record parses and validates, manifest matches objects (sizes), SHA-256 re-hashed, all four labels, size limits, member paths, depositor authorised for the strand (`PROMOTER_AUTHORISED`), destination record sane. Members first, record last, verify, then delete markers in staging (control object last).
