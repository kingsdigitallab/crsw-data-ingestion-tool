...
```
{strand}/{project}/{sensitivity}/{state}/{original-filename}

rs2/csac/green/2_final/csac-clean-2025.csv
rs2/csac/green/2_final/csac-clean-2025.csv.meta.json
```

*(r3: sensitivity sits above state so each sensitivity level forms a
contiguous prefix for access policies — see `DEPOSIT_TOOL_SPEC_R3.md`.)*
...
| `project` | Open list. Offer existing prefixes from the bucket first (numbered picker, r4); new names are normalised (lowercase, spaces to hyphens, strip outside `[a-z0-9-]`), warned on near-matches, and need explicit confirmation. The listing uses `rclone lsjson --dirs-only` — shallow and equivalent to r4's `lsf` sketch; don't "fix" it back. |
...
python deposit.py FILE_OR_GLOB [FILE_OR_GLOB ...] [options]
  ...
  --bucket crsw           target bucket (default: crsw — confirmed r4)
```

Anything not supplied is prompted for. Fully-flagged `--dry-run` is how researchers check before committing and how support reproduces a problem (r4: a `--project` new to the strand prints a note under `--dry-run` instead of prompting, and is confirmed once on a real run).
...
4. **Prompt batch metadata:** strand, project, state, sensitivity, domain, version, coverage dates, subjects, abstract. (r4: project is a numbered pick from the strand's existing prefixes, `n` for a new name — normalised, near-match-warned, confirmed.)
